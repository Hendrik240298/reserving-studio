...
| LDF Compare | Implemented v1 | The user asks to compare LDF vectors across explicit reserving scenario settings. | `harness/tools/ldf_compare.md` |
...
- Use **LDF Compare** for requests like "compare these reserving settings", "show how drops change the LDF vector", or "compare LDFs for multiple scenarios".
...
uv run python -m harness.cli ldf-compare --config examples/config_quarterly.yml --scenarios-file examples/ldf_compare_scenarios.yml
